...
def get_generated_reports():
    """Fetches the list of reports generated by the current user."""
    config = load_trends_config()
    db_settings = config.get('database')
    conn = None
    reports = []
    try:
        ...
        cursor.execute(
            """
            SELECT id, report_name, report_description, annotations, generation_timestamp, run_id
            FROM generated_ai_reports
            WHERE generated_by_user_id = %s
