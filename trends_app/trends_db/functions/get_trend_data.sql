...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'first_run', MIN(run_date),
        'last_run', MAX(run_date)
    ) INTO v_summary_data
    FROM health_check_runs
    WHERE company_id = p_company_id
      AND run_date > NOW() - (p_days || ' days')::INTERVAL;
    ...
    FROM (
        ...
        SELECT 
            ...
            MIN(hcr.run_date) as first_seen,
            MAX(hcr.run_date) as last_seen,
            COALESCE(ARRAY_AGG(DISTINCT htr.module_name ORDER BY htr.module_name) FILTER (WHERE htr.module_name IS NOT NULL), '{}') as affected_modules,
            EXISTS(
