...
def run(connector, settings):
    """
    Check for known CVEs affecting Kafka.

    Args:
        connector: Kafka connector (must have CVECheckMixin)
    ...
    builder.text(f"- **Kafka Version:** {version_string}")
    builder.text(f"- **Major Version:** {major_version}")
    builder.text(f"- **Environment:** {env_name}")
    builder.blank()

    # Check core Kafka CVEs
    builder.h4("Kafka Core Vulnerabilities")

    logger.info(f"Querying NVD for Kafka {version_string} CVEs")
    ...
    if total_cves == 0:
        builder.success(f"✅ No known CVEs found for Kafka {version_string}")
        builder.blank()
        builder.text("*Note:* This indicates no CVEs specifically match this exact version in NVD.")
        builder.text("Always review Apache Kafka security announcements for the latest information:")
        builder.text("https://kafka.apache.org/project-security")
    else:
        ...
        if critical_count > 0:
            builder.critical(f"🔴 **{critical_count} Critical CVE(s)** found for Kafka {version_string}")
        elif high_count > 0:
            builder.error(f"🟠 **{high_count} High severity CVE(s)** found for Kafka {version_string}")
        elif medium_count > 0:
            builder.warning(f"🟡 **{medium_count} Medium severity CVE(s)** found for Kafka {version_string}")
        else:
            builder.note(f"**{total_cves} Low severity CVE(s)** found for Kafka {version_string}")
        ...
        if cve_list:
            ...
            builder.note(
                "**About These Results:** NVD's CPE matching may include CVEs for Kafka "
        ...
        if total_cves > 0:
            recommendations.extend([
                f"Upgrade to latest Kafka {major_version}.x minor version to address known vulnerabilities",
                "Review detailed CVE information at NVD: https://nvd.nist.gov",
                "Check Apache Kafka security announcements: https://kafka.apache.org/project-security",
                "Subscribe to Kafka security mailing list for updates"
            ])
