"""
Disk usage check for Cassandra brokers.

Queries Cassandra for actual log directory paths, then checks disk usage
...
def run_check_disk_usage(connector, settings):
    """
    Checks disk usage on all Cassandra brokers via SSH.
    
    First queries Cassandra to discover actual log directories configured
    ...
    try:
        ...
        # === STEP 1: QUERY Cassandra FOR LOG DIRECTORIES ===
        ...
        if not log_dirs_raw or isinstance(log_dirs_raw, dict) and 'error' in log_dirs_raw:
            builder.warning(
                "Could not retrieve log directory information from Cassandra.\n"
            ...
            if broker_log_dirs:
                builder.para("**Cassandra Log Directories Discovered:**")
        ...
        for ssh_host in connector.get_ssh_hosts():
            ...
            log_dirs = broker_log_dirs.get(broker_id, ['/data/cassandra'])  # Fallback
            
            if not log_dirs:
                log_dirs = ['/data/cassandra']  # Ultimate fallback
