"""
OpenSearch CVE Vulnerability Check
...
def run(connector, settings):
    """
    Check for known CVEs affecting OpenSearch.

    Args:
        connector: OpenSearch connector (must have CVECheckMixin)
    ...
    builder.text(f"- **OpenSearch Version:** {version_string}")
    builder.text(f"- **Major Version:** {major_version}")
    builder.text(f"- **Environment:** {env_name}")
    builder.blank()

    # Check core OpenSearch CVEs
    builder.h4("OpenSearch Core Vulnerabilities")

    logger.info(f"Querying NVD for OpenSearch {version_string} CVEs")
    ...
    structured_data = {
        ...
        'data': {
            'opensearch_version': version_string,
    ...
    if total_cves == 0:
        builder.success(f"✅ No known CVEs found for OpenSearch {version_string}")
        builder.blank()
        builder.text("*Note:* This indicates no CVEs specifically match this exact version in NVD.")
        builder.text("Always review Apache OpenSearch security announcements for the latest information:")
        builder.text("https://opensearch.org/security")
    else:
        ...
        if critical_count > 0:
            builder.critical(f"🔴 **{critical_count} Critical CVE(s)** found for OpenSearch {version_string}")
        elif high_count > 0:
            builder.error(f"🟠 **{high_count} High severity CVE(s)** found for OpenSearch {version_string}")
        elif medium_count > 0:
            builder.warning(f"🟡 **{medium_count} Medium severity CVE(s)** found for OpenSearch {version_string}")
        else:
            builder.note(f"**{total_cves} Low severity CVE(s)** found for OpenSearch {version_string}")
        ...
        if cve_list:
            ...
            builder.note(
                "**About These Results:** NVD's CPE matching may include CVEs for OpenSearch "
        ...
        if total_cves > 0:
            recommendations.extend([
                f"Upgrade to latest OpenSearch {major_version}.x minor version to address known vulnerabilities",
                "Review detailed CVE information at NVD: https://nvd.nist.gov",
                "Check Apache OpenSearch security announcements: https://opensearch.org/security",
                "Subscribe to OpenSearch security mailing list for updates"
            ])
