...
def run(connector, settings):
    """
    Check for known CVEs affecting ClickHouse.

    Args:
        connector: ClickHouse connector (must have CVECheckMixin)
    ...
    builder.text(f"- **ClickHouse Version:** {version_string}")
    builder.text(f"- **Major Version:** {major_version}")
    builder.text(f"- **Environment:** {env_name}")
    builder.blank()

    # Check core ClickHouse CVEs
    builder.h4("ClickHouse Core Vulnerabilities")

    logger.info(f"Querying NVD for ClickHouse {version_string} CVEs")
    ...
    if total_cves == 0:
        builder.success(f"✅ No known CVEs found for ClickHouse {version_string}")
        builder.blank()
        builder.text("*Note:* This indicates no CVEs specifically match this exact version in NVD.")
        builder.text("Always review Apache ClickHouse security announcements for the latest information:")
        builder.text("https://clickhouse.com/docs/en/operations/security")
    else:
        ...
        if critical_count > 0:
            builder.critical(f"🔴 **{critical_count} Critical CVE(s)** found for ClickHouse {version_string}")
        elif high_count > 0:
            builder.error(f"🟠 **{high_count} High severity CVE(s)** found for ClickHouse {version_string}")
        elif medium_count > 0:
            builder.warning(f"🟡 **{medium_count} Medium severity CVE(s)** found for ClickHouse {version_string}")
        else:
            builder.note(f"**{total_cves} Low severity CVE(s)** found for ClickHouse {version_string}")
        ...
        if cve_list:
            ...
            builder.note(
                "**About These Results:** NVD's CPE matching may include CVEs for ClickHouse "
                "client libraries, connectors, or older versions. Review each CVE's 'Description' "
        ...
        if total_cves > 0:
            recommendations.extend([
                f"Upgrade to latest ClickHouse {major_version}.x minor version to address known vulnerabilities",
                "Review detailed CVE information at NVD: https://nvd.nist.gov",
                "Check Apache ClickHouse security announcements: https://clickhouse.com/docs/en/operations/security",
                "Subscribe to ClickHouse security mailing list for updates"
            ])

        # Environment-specific recommendations
        if env_name == 'CONFLUENT':
            recommendations.append("Contact Confluent Support for managed patching schedules and available versions")
            recommendations.append("Review Confluent Security Advisories: https://www.confluent.io/security/")
            ...
            recommendations.append("Contact AWS Support for managed patching schedules")
            recommendations.append("Review AWS MSK version compatibility and upgrade options")
